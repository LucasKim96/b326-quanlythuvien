...
export const updateBook = async (updatedBook) => {
  try {
    await axios.put(
      `http://localhost:5000/api/sach/edit/${updatedBook._id}`,
      updatedBook
    );
  } catch (error) {
...
export const addBook = async (newBook) => {
  try {
    const response = await axios.post(
      "http://localhost:5000/api/sach/add",
      newBook
    );
...
export const deleteBook = async (id) => {
  try {
    await axios.delete(`http://localhost:5000/api/sach/delete/${id}`); // Đường dẫn API thêm nhà xuất bản
  } catch (error) {
...
export const searchBooksByName = async (name) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/sach/timkiem/tensach?TenSach=${name}`
    );
...
export const searchBooksByAuthor = async (author) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/sach/timkiem/tacgia?TacGia=${author}`
    );
    return response.data; // Trả về danh sách sách tìm thấy
  } catch (error) {
